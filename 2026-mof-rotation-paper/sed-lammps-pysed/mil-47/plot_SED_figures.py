...
def plot_sed(out_files_name, plot_cutoff_freq = None, plot_interval = 5, vmin = None, vmax = None, if_show_figures=False):
    ...
    log = True
    color = 'Spectral'       # 'inferno', 'jet', 'Spectral', 'RdYlGn' , 'coolwarm', 'rainbow' (https://matplotlib.org/stable/tutorials/colors/colormaps.html)
    ...
    fig, ax = plt.subplots()
    fig.set_size_inches(6.5, 9, forward=True)       # Control the size of the output image
    ...
    ax.set_yticks(ids)
    ax.set_yticklabels([f"{freq:.1f}" for freq in freqs], fontsize=16)  # 保留1位小数
    ...
    ax.set_xlabel(r'$\mathbf{q}$',labelpad = 5.0,fontweight='normal',fontsize=fontsize)
    ax.set_ylabel(r'$\mathbf{Frequency\ (THz)}$', labelpad = 8.0, fontweight='normal', fontsize=fontsize)
    ax.tick_params(bottom=False, top=False, left=True, right=False)

    fig.suptitle(r'$\Phi(\mathbf{q},\ \omega)$', x=0.48, y=0.95, fontweight='normal', fontsize=fontsize)
...
if __name__ == "__main__":
    ...
    out_files_name = 'MIL-47'
    plot_cutoff_freq = 30              # THz
    plot_interval = 5                # THz
    vmax = -9                          # For SED image (high)
    vmin = -18                         # For SED image (low)

    plot_sed(out_files_name, plot_cutoff_freq, plot_interval=plot_interval, vmax=vmax, vmin=vmin, if_show_figures=True) ### vmax=vmax, vmin=vmin,
